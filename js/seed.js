...
import { collection, addDoc, serverTimestamp, doc, query, where, getDocs } from "https://www.gstatic.com/firebasejs/10.12.0/firebase-firestore.js";
...
onAuthStateChanged(auth, async (user) => {
  if (!user) {
    document.getElementById('status').innerHTML = '<p class="text-red-600 text-center">Please <a href="/login.html" class="text-blue-600">login</a> first.</p>';
    return;
  }
  currentUser = user;
  ...
  if (!userSnap.empty) {
    userOrgId = userSnap.docs[0].data().orgId;
    document.getElementById('status').innerHTML = `<p class="text-green-600 text-center">Logged in. Org: ${userOrgId.substring(0, 8)}...</p>`;
  }
});
...
document.getElementById('seedBtn').addEventListener('click', async () => {
  ...
  const log = (msg) => { status.innerHTML += `<p class="text-xs text-slate-500">${msg}</p>`; };
